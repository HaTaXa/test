...
var comments_xcomment =
	comments_type.toLowerCase() == "xcomment" ? true : false;
...
function doXcomment() {
	...
	$(window).bind("unload", function () {
		setCookie("commentsOpen", commentsOpen, "seconds", 3);
	});
	commentsOpen = getCookie("commentsOpen") === "open" ? "open" : "closed";
	if (!$("span.xcSpanNoMessages").length > 0) {
		if ($("span.xcSpanPagination").length > 0) {
			(function () {
				var b = parseInt(
					$(
						$("a.xcPaginationLinks")[$("a.xcPaginationLinks").length - 1]
					).html(),
					10
				);
				var a = parseInt(
					$("span.xcSpanCurrentPageNum").html().replace(/\(|\)/g, ""),
					10
				);
				b = b > a ? b : a;
				$("span.comments").html(b + " " + comments_pages);
			})();
		} else {
