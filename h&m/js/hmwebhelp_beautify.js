...
function chromeWarning(a) {
	if (a) {
		...
		var c = (b.indexOf("applewebkit/") != -1) && (b.indexOf("khtml") > -1) && (b.indexOf("chrome") > -1);
		if (c && a) {
			return true
		} else {
			return false
...
(function () {
	...
	hmBrowser.agent = f;
	hmBrowser.oldMobile = ((f.indexOf("series60") != -1) || (f.indexOf("symbian") != -1) || (f.indexOf("windows ce") != -1));
	var b = navigator.userAgent.match(/blackberry(.*?\(khtml, like gecko\) version\/(\d)\.\d.\d.[\d]{1,4}.*?mobile safari)?/ig);
	if (b != null && b[1] == null) {
	...
	hmBrowser.touchOS = ((c("touchstart") || "ontouchstart" in window) || (navigator.maxTouchPoints > 0) || (navigator.msMaxTouchPoints > 0));
	hmBrowser.windows7 = /windows nt 6\.1/i.test(f) && /win32/gi.test(navigator.platform);
	hmBrowser.windows8 = /windows nt 6\.[23]/i.test(f) && /win32/gi.test(navigator.platform);
	hmBrowser.windowsTouchIe = (!hmBrowser.wp7 && !hmBrowser.wp8 && /windows nt 6\.[23].+?trident\/[67].+?touch/ig.test(f) && hmBrowser.touchOS);
	...
	hmBrowser.android = (/android/gi.test(f) || (!hmBrowser.iOS && !hmBrowser.oldMobile && !hmBrowser.wp7 && !hmBrowser.wp8 && hmBrowser.touchOS && !hmBrowser.desktopOS && !hmBrowser.windowsTouch));
	hmBrowser.androidTablet = (hmBrowser.android && (!/mobile/gi.test(f) || /tablet/gi.test(f)));
...
var jsCSS = "div#idToolbar, div#hmtabscontrols {display: block;}\ntable#idToolbarTable {display: table;}";
...
function doPermalink(a) {
	document.onselectstart = function () {};
	var f = hmWebHelp.tabsapi.getCurrentPane().find("iframe").contents().find("title").text();
	var g = hmWebHelp.tabsapi.getCurrentPane().find("iframe").attr("src");
	if (hmnavpages.cachefix) {
		g = g.replace(/\?cachehash=\d*?$/img, "")
	}
	...
	switch (a) {
		...
		default:
			...
			$("div#unclicker").bind("click.closeperma", function () {
				closePermalink()
			}).show();
			var d = 400;
			...
			$("textarea#plinkBox").css("height", (Math.round(e.length / 30) * 17) + "px");
			$("textarea#plinkBox").css("width", d + "px").val(e);
...
function initClipboard() {
	if ((navigator.platform.indexOf("iPad") === -1 && navigator.platform.indexOf("iPhone") === -1) && hmWebHelp.isFlash) {
		...
		a.on("ready", function (b) {
			a.on("copy", function (c) {
				c.clipboardData.setData("text/plain", document.getElementById("plinkBox").value)
			});
...
function browserPlatform(a) {
	...
	if (a && (hmBrowser.androidPhone || hmBrowser.iPhone || hmBrowser.wp8 || hmBrowser.wp7 || hmBrowser.bb)) {
		if (mobRe.smart != "") {
...
function openWindow(c, a, b) {
	this.win = c;
	this.tabID = a;
	this.liID = a.replace(/(.*?)Link$/mg, "li$1");
	this.tool = b
}
var hmTabSlider = {
	sliderWidth: 0,
	...
	updateSlider: function () {
		...
		for (var a = 0; a < hmTabSlider.visibleTabs.length; a++) {
			b = b + "index " + a + ": " + hmTabSlider.visibleTabs[a] + " - " + hmTabSlider.navTabs[a] + "\n"
		}
		hmTabSlider.sliderWidth += 2;
		$("ul.tabs a").each(function (e) {
			var f = e == 0 ? hminfo.currtopic + ":&nbsp;" + $(this).attr("data") : (e == 1 || e == 2) ? $(this).text() : $(this).attr("data");
			if ($(this).is(":visible")) {
				$("ul#tabsList").append("<li><a id='tabList" + e + "' href='#' onclick='hmTabSlider.goToTab(" + e + ",true)'>" + f + "</a></li>")
			}
		});
		...
		$("ul.tabs a").each(function (d) {
			...
			if (e && e.indexOf("current") > -1) {
				...
				if ($("ul#tabsList a").length > 0) {
					$("ul#tabsList a[id='tabList" + d + "']").css("font-weight", "normal")
				}
			}
		})
	},
	maxTab: function () {
		...
		a = parseInt(a) + 2;
		a = $("ul.tabs a").not(":hidden").length - 1 > a ? a : $("ul.tabs a").not(":hidden").length - 1;
		return a
	},
	...
	goToTab: function (a, b) {
		...
		if (hmTabSlider.sliderWidth > $("div#hmtabswindow").innerWidth()) {
			...
			if (hmTabSlider.sliderWidth < $("div#hmtabswindow").innerWidth() && d.left < 0) {
	...
	nextValidTab: function (d, c) {
		...
		if (c) {
			while (!$("ul#topictabs li[tabnum='" + d + "']").is(":visible") && d < a) {
				d++
			}
		} else {
			while (!$("ul#topictabs li[tabnum='" + d + "']").is(":visible") && d > 0) {
				d--
			}
		}
		if (!$("ul#topictabs li[tabnum='" + d + "']").is(":visible") || d > a || d < 0) {
			d = b - 1
		}
		if (d < 0) {
			d = 0
...
var hmWebHelp = {
		...
		if (navigator.platform != "iPad" && navigator.platform != "iPhone") {
			if (window.ActiveXObject) {
				...
				if (navigator.plugins && navigator.plugins.length) {
					d = navigator.plugins["Shockwave Flash 2.0"] || navigator.plugins["Shockwave Flash"] ? true : false
				} else {
	...
	doTabsCookie: function (c) {
		...
		switch (c) {
			case "getDefaultTopic":
				...
				break;
			case "setTabsCookie":
				var d = hmWebHelp.currentPage.substr(0, hmWebHelp.currentPage.lastIndexOf("."));
				if (!d || d === "") {
					return
				}
				a = d;
	...
	tbToolAdjust: function () {
		...
		setTimeout(function () {
			...
			function b() {
				var e = 0;
				$("div#idSearch:visible,.nav-icon-box-left:visible,.nav-icon-box-right:visible").each(function () {
					e += $(this).outerWidth() + a
				});
				return e
			}
	...
	doSocial: function () {
		...
		if ($(a).css("visibility") == "hidden") {
			$(c).bind("click", function (d) {
				$("div.googleplusone-button iframe, div.twitter-button iframe", hmWebHelp.currentDoc).hide();
				$(a).css("visibility", "hidden");
				$(c).hide().unbind("click")
			}).show();
			$("div.googleplusone-button iframe, div.twitter-button iframe", hmWebHelp.currentDoc).show();
	...
	writeTopic: function () {
		...
		document.write('<iframe name="hmcontent" id="hmcontent" class="scrollpane" src="' + c + '" title="Content Tab" height="100%" width="100%" frameborder="0" scrolling="auto"></iframe>')
	},
	writeTOC: function () {
		...
		document.write('<iframe name="hmnavigation" id="hmnavigation" class="scrollpane" src="' + a + '" title="Navigation Pane" height="100%" width="100%" frameborder="0" scrolling="auto"></iframe>')
	},
	...
	maxZindex: function () {
		var a = document.getElementsByTagName ? document.getElementsByTagName("*") : document.all;
		return a.length
	},
	...
	clipTitle: function (e, b) {
		...
		if (e.length > b) {
			...
			if (f < 5) {
				return trimString(e.substr(0, trimString(d).lastIndexOf(" ") + a.length + 1)) + "&hellip;"
			} else {
				return trimString(e.substr(0, trimString(d).lastIndexOf(" "))) + "&hellip;"
			}
		} else {
			return e
		}
	...
	resizeNav: function (f, d) {
		...
		if ($("img#navshowhide").offset().left > 100) {
			var c = (!document.all && !window.opera) ? f.clientX - d : event.clientX - d;
	...
	mailFB: function (c) {
		...
		if (f.tVars.mailUrl !== "") {
			...
			if (l) {
				l = trimString(l);
				l = l.replace(/^\?/, "").replace(/&amp;/g, "&").replace(/&gt;/g, ">").replace(/&lt;/g, "<").replace(/\%20/g, " ").replace(/&quot;/g, '"').replace(/&apos;/g, "'");
				a = a + "?" + l
			}
			a = encodeURI(a);
		...
		var i = function (q) {
			...
			var p = "";
			q = q.replace(/^\$\$/g, "").replace(/\$\$$/, "").replace(/\/\//g, "/").replace(/\"/g, "");
			q = q.split("/");
			if (q.length === 4) {
				p = '"' + q[0] + '" <' + q[1] + "@" + q[2] + "." + q[3] + ">"
			} else {
		...
		if (!c) {
			var n = "mailto:" + (hmnavpages.fb_unicode ? encodeURIComponent(j) : escape(j)) + "?subject=" + g;
			var m = "&body=Ref:%20" + b + "%20ID:%20" + h + "%0A%0D" + e + "%0A%0D"
		} else {
			var n = "mailto:" + (hmnavpages.fb_unicode ? encodeURIComponent(o) : escape(o)) + "?subject=" + f.tVars.simplesubject;
	...
	ffFix: function () {
		...
		if (a.indexOf("r2d2") > -1 || a.indexOf("nav=") > -1 || a.indexOf("search=") > -1 || a.indexOf("index=") > -1) {
			return
		} else {
			a = a == "" ? "?r2d2" : a + "&r2d2";
	...
	hmInit: function () {
		...
		hmnavpages.autoTabs = (function () {
			...
			if (j.substr(2, 9) === "AUTO_TABS") {
				...
				j = j.replace(/ /g, "").replace(/&nbsp;/g, "").replace(/,$/, "").toLowerCase();
				return j.split(",")
			}
		})();
		if ((hmnavpages.userReload || hmnavpages.autoTabs) && hmWebHelp.server) {
			...
			if ((!hmWebHelp.tabsCookie || hmWebHelp.tabsCookie.length < 1) && hmnavpages.autoTabs && hmWebHelp.fileExists(hmnavpages.autoTabs[0] + hmWebHelp.textn)) {
				...
				if (hmnavpages.userReload && hmWebHelp.fileExists(hmWebHelp.tabsCookie[0] + hmWebHelp.textn)) {
		...
		if (hmpldata.show) {
			$("body").prepend('<div id="permalink" class="popups"><table cellspacing="3" cellpadding="0" width="400" height="50"><tr><td align="left"><div id="permalink_box" style="position:relative">&nbsp;<input type="button" id="selectPermalink" value="' + hmpldata.select + '" /></div></td><td align="center"><input type="button" id="bookmarkPermalink" value="' + hmpldata.bookmark + '" onClick="doPermalink(\'bookmark\')" /></td><td align="right"><input type="button" id="closePermalink" value="' + hmpldata.close + '" onClick="doPermalink(\'close\')" /></td></tr><tr id="offlinemessage"><td colspan="3" align="center"> <p>' + hmpldata.manualcopy + '</p></td></tr><tr><td colspan="3" align="center"> <textarea id="plinkBox" readonly="readonly"></textarea></td></tr></table></div>');
			$("td#idTopicNavTD").prepend('<div class="nav-icon-box-right" id="idPermalinkTool"><span id="idNavIconPermalink" onclick="doPermalink(hmnavpages.top);"><img id="idPermalinkOn" class="nav-icon" src="permalink.png" alt="' + hmpldata.copy + '" title="' + hmpldata.copy + '"/></span><img id="idPermalinkOff" src="permalinkoff.png" /><br /><span id="idPermalinkText" class="nav-text-on">' + hmpldata.title + "</span></div>")
		}
		$("input[id*='Permalink']").css("cursor", "pointer");
		$("div#hmnavbox").prepend('<div id="hmdragdivider" /><div id="hmnavhandle"><a href="javascript: void(0);" onclick="hmWebHelp.showHideNav()"><img src="nav_close.png" id="navshowhide" alt="' + hminfo.hidenav + '" title="' + hminfo.hidenav + '" border="0"/></a></div>');
		...
		$("div#hmtopicbox").append('<div id="hmindexbox"><iframe name="hmindex" id="hmindex" class="scrollpane" src="' + a + '" title="Index Tab" height="100%" width="100%" frameborder="0" scrolling="auto"></iframe></div><div id="hmsearchbox"><iframe name="hmsearchframe" id="hmsearchframe" class="scrollpane" src="' + c + '" title="Search Tab" height="100%" width="100%" frameborder="0" scrolling="auto"></iframe>');
		...
		$("div#hmdragdivider").mousedown(function (k) {
			if (!hmWebHelp.navHidden) {
				...
				$("div#unclicker").bind("mousemove", function (l) {
		...
		if (navigator.platform.indexOf("iPad") < 0 && !/Android/.test(navigator.userAgent)) {
			$("img.navicon").mouseover(function () {
				$(this).toggleClass("nav-icon naviconH")
			}).mouseout(function () {
				$(this).toggleClass("nav-icon naviconH")
			});
		...
		$("input#idInputSearch").keyup(function (j) {
			if (j.keyCode == 13 && $("input#idInputSearch").val().length > 0 && hmWebHelp.qSearch) {
		...
		$("img#toggleTabMenu").bind("click", function (j) {
			...
			if ($("div#tabsMenu").is(":visible")) {
				$("div#unclicker").show().bind("click.tabmenu", function () {
					$("div#tabsMenu").slideUp("fast");
		...
		var g = setInterval(function () {
			if (hmWebHelp.tocDoc != 0 && hmWebHelp.idxDoc != 0 && hmWebHelp.schDoc != 0 && hmWebHelp.mainDoc != 0 && hmWebHelp.newTabReady) {
				clearInterval(g);
		...
		$("#hmcontent").bind("load", function () {
				...
				if (j) {
					if (hmWebHelp.external) {
						...
						if (this.contentWindow.tVars != null) {
							...
							hmWebHelp.external = $("a[href$='" + n + "']", hmWebHelp.tocDoc).attr("href")
						}
					}
				}
			} catch (l) {
			...
			if (k && !hmWebHelp.external && n) {
				...
				if (hmWebHelp.external && n) {
					m = $("a[href='" + hmWebHelp.external + "']", hmWebHelp.tocDoc).children("span").text();
	...
	initHeadMenu: function () {
		...
		if (!(("ontouchstart" in window) || (navigator.maxTouchPoints > 0) || (navigator.msMaxTouchPoints > 0))) {
			...
			$("ul.topnav > li:has(ul) > a").on("click", function (b) {
				b.preventDefault();
				$("div#unclicker").bind("click.closemenu", function () {
	...
	newTab: function (d) {
		var c = d ? d : hmWebHelp.currentPage.substr(0, hmWebHelp.currentPage.lastIndexOf("."));
		var e = c + hmWebHelp.currentPage.substr(hmWebHelp.currentPage.lastIndexOf("."));
		...
		$("li#searchtab").after('<li class="hmtabs" id="' + c + '" hpage="' + e + f + 'style="visibility: hidden;"><img class="closetab" src="closetaboff.png" alt="' + hminfo.tabclose + '" title="' + hminfo.tabclose + '"/><a href="#" id="' + g + '"><span>-</span></a></li>');
		$("div#hmsearchbox").after('<div class="hmnewtabbox" id="' + b + '"><iframe name="' + a + '" id="' + a + '" class="scrollpane" src="' + e + '" title="New Tab" height="100%" width="100%" frameborder="0" scrolling="auto"></iframe></div>');
		$("ul.tabs").tabs("div#hmtopicbox > div");
		hmWebHelp.tabsapi = $("ul.tabs").data("tabs");
	...
	initTab: function (a, c, b) {
		...
		$("img.closetab", a).mouseover(function () {
			$(this).css("cursor", "pointer").attr("src", "closetabon.png")
		}).mouseout(function () {
			$(this).attr("src", "closetaboff.png")
		}).click(function () {
	...
	getCurTabDoc: function (b) {
		...
		if (c > 0 && c < 3) {
			...
			var a = hmWebHelp.tabsapi.getCurrentTab().attr("id").replace("Link", "Frame");
			if (a == "topictablink") {
				a = "hmcontent"
			}
		}
	...
	undockTab: function () {
		...
		if (c == 2) {
			...
			a = window.open(hmnavpages.sch, "hmsearchwindow", "toolbar=0,scrollbars=0,location=0,statusbar=0,menubar=0,titlebar=0,resizable=1,width=" + d + ",height=" + i + "")
		} else {
			if (c == 1) {
				a = window.open(hmnavpages.idx, "hmindexwindow", "toolbar=0,scrollbars=0,location=0,statusbar=0,menubar=0,titlebar=0,resizable=1,width=" + d + ",height=" + i + "")
			} else {
				if (c > 2) {
					var g = b.replace(/(.*?)Link$/mg, "iframe$1Frame");
					var f = hmnavpages.cachefix ? $(g).attr("src") + "&toc=0" : $(g).attr("src") + "?toc=0";
					var h = b.replace(/#(.*?)Link$/mg, "$1Window");
					a = window.open(f, h, "toolbar=0,scrollbars=1,location=0,statusbar=0,menubar=0,titlebar=0,resizable=1,width=" + d + ",height=" + i + "");
					lazysync(hmWebHelp.currentPage)
				}
			}
		}
